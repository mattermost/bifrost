...
release-notes --org mattermost --repo bifrost --start-sha "${LATEST_RELEASE_NEXT_COMMIT_SHA}" --end-sha "${FUTURE_RELEASE_SHA}"  --output ./build/_output/docs/relnote.md --required-author "" --branch main
...
mv  ./build/_output/bin/bifrost ./build/_output/bin/bifrost-linux

hub release create -d -F ./build/_output/docs/relnote.md -a ./build/_output/bin/bifrost-linux -a ./build/_output/bin/bifrost-darwin ${CIRCLE_TAG}
rm -rf ./build/_output/